...
- Phase 9I disabled backend OAuth architecture skeleton with typed no-op contracts and tests.
- Phase 9J receipt source-provider UX and metadata unification for current local/mock import paths.
...
Phase 10A keeps the build pipeline on Vite:
...
Local production preview uses Vite's preview server:

```text
npm run preview -- --host 127.0.0.1 --port 4173
  -> serve dist/
  -> verify index.html, built assets, and manifest.webmanifest
```

Production build output is static. A deployment target should serve `dist/`, use HTTPS for PWA install behavior, and fall back unknown SPA paths to `index.html`. If the app is hosted under a subpath instead of `/`, Vite `base` must be configured before rebuilding and rechecking preview.

PWA metadata lives in:

- `index.html` for title, description, theme color, viewport, favicon, and manifest link.
- `public/manifest.webmanifest` for app name, short name, description, start URL, scope, display mode, colors, categories, language, and icon placeholder.
- `public/app-icon.svg` and `public/favicon.svg` for current placeholder icons.

Phase 10A does not add a service worker or offline asset cache. The app is local-first because finance records live in browser IndexedDB and file import/export stays browser-local. Static app assets are not guaranteed to load offline until a later explicit service-worker phase implements and validates that behavior.
...
src/
  ...
  google-integration/
    ...
    googleOAuthBackendSkeleton.test.ts
    googleOAuthBackendSkeleton.ts
...
- a future implementation sequence after release-gate approval: backend OAuth behind disabled flags, local/mock source-provider QA, narrow Drive/Docs picker, Gmail selected-message import, optional scheduled sync, then production hardening.
...
Phase 9I adds a typed, frontend-safe architecture boundary for a future backend-backed Google OAuth implementation. It does not add a backend runtime, OAuth route, Google API client, credential exchange, token store, revocation call, provider data deletion runtime, source sync, scheduled sync, dependency, or UI behavior change.
...
Disabled Phase 9I boundary:

```text
Phase 9D backend readiness flags
  -> buildGoogleOAuthBackendSkeletonConfig
  -> googleOAuthBackendEndpointContracts
  -> DisabledGoogleOAuthBackendBoundaryClient
  -> typed disabled responses only
```

Future backend contracts are named for OAuth start, OAuth callback, provider status, provider disconnect, provider revoke, and source sync. The endpoint contract metadata stays disabled and records that no provider credentials are stored.

Phase 9I invariants:

- `endpointCallsAllowed` is always `false`.
- `networkCallsAllowed` is always `false`.
- `tokenStorageAllowed` is always `false`.
- Requested backend auth, revocation, or sync flags can move the skeleton to `requested_but_blocked`, but cannot enable calls.
- The disabled boundary client does not call its optional network adapter.
- Disabled responses do not expose access tokens, refresh tokens, id tokens, client secrets, authorization URLs, authorization-code values, provider sessions, sync cursors, or source text.
- Source sync returns zero synced candidates and does not fetch provider text or store cursors.

Existing product behavior remains unchanged. Phase 9B mock sources, Phase 9F local Drive/Docs files, and Phase 9G local Gmail paste/files remain the only Google-like runtime import behavior, and all still create receipt drafts only.

## Phase 9J Receipt Source-Provider UX and Metadata

Phase 9J unifies the current receipt source import UX and source metadata display. It does not add real provider access, OAuth, backend runtime, token storage, provider revocation calls, source sync, scheduled sync, real AI calls, dependencies, or accounting behavior changes.
...
Current source-provider UX model:

```text
receiptSourceProviderUxEntries
  -> ReceiptsPage source overview cards
  -> source-specific local/mock import forms
  -> source preview panels
  -> saved draft source metadata
  -> draft review source metadata
```

Current source-provider entries are manual paste, AI simulator, local Gmail, local Drive/Docs, and mock Google. Each entry is labeled with a local/mock status and still writes editable receipt drafts only.

Source metadata display uses existing persisted fields only. Saved drafts and draft review show source type, source title or filename, sender/owner when available, imported timestamp from fetched/extracted/created time, provider/model/source id details when available, and duplicate tracking status based on source id and/or content hash when available. Manual pasted drafts fall back to manual paste, merchant/title, created time, and duplicate unavailable status.

Duplicate behavior remains in service/repository boundaries. Phase 9J surfaces mock source duplicate status in the Receipts UI and duplicate warnings returned by import actions, but it does not change duplicate matching semantics or silently overwrite local drafts, confirmed receipts, linked transactions, analytics, recurring expenses, FX settings, backup/restore state, or CSV data.

A focused repository test now imports through every current source path before confirmation and verifies all results remain `draft` while transactions, confirmed receipts, confirmed receipt items, recurring expenses, currency settings, and Dashboard overview remain unchanged. Human review and explicit receipt confirmation remain the only path to one final receipt, final receipt items, one linked transaction, and Dashboard impact.

## Phase 9K Source-Provider End-to-End QA and Release Candidate

Phase 9K is a local/mock QA and release-candidate pass for the existing receipt source-provider system. It does not add real provider access, OAuth, backend runtime, token storage, provider revocation calls, source sync, scheduled sync, real AI calls, dependencies, or accounting behavior changes.

The QA evidence spans the current source-provider matrix:

```text
manual paste/parser
manual AI simulator
local Gmail manual paste/file import
local Drive/Docs selected-file import
mock Google samples
  -> validated draft creation only
  -> review/edit remains draft-only
  -> explicit confirmation creates final receipt records
  -> Dashboard and item analytics update only after confirmation
```

Automated release-candidate coverage includes a representative local Gmail source-provider path from import through review, confirmation, Dashboard/item analytics, JSON backup/restore, and confirmed receipt item CSV export. That test proves source metadata is preserved on the confirmed draft and final receipt, JSON backup/restore preserves the metadata, and CSV export remains read-only while surfacing confirmed receipt source evidence.

Existing invalid extraction tests cover manual AI, local Gmail, local Drive/Docs, and mock Google provider paths. Existing duplicate tests cover mock Google, local Gmail, local Drive/Docs, and Receipts UI duplicate messages/status. Phase 9K documents this as the release-candidate baseline before any future real provider work.
